...
namespace Mood.Draw
{
    public class SanphamDraw
    {
        ...
        public List<Sanpham> getProduct(int top)
        {
            return db.Sanphams.Where(x => x.Status == true).Take(top).ToList();
        }
        ...
        public List<Sanpham> listProductByPrice(String sortType)
        {
            List<Sanpham> temp = null;
            if (sortType.Equals("asc"))
            {
                /*Where(x => x.PriceSale > 0 && x.Status == true).OrderByDescending(x => x.PriceSale)*/
                temp = db.Sanphams.OrderBy(x => x.PriceSale).Where(x => x.Status == true && x.PriceSale != null).ToList();
            }
            if (sortType.Equals("desc"))
            {
                temp = db.Sanphams.OrderByDescending(x => x.PriceSale).Where(x => x.Status == true && x.PriceSale != null).ToList();
            }
            return temp;
        }
